...
<body class="bg-gray-50 font-sans">
    ...
    <main class="md:ml-64 xl:ml-72 2xl:ml-72">
        ...
        <div class="p-6 space-y-6">
            ...
            @php
                $money = fn (string $amount) => bccomp($amount, '0', 2) === 0 ? '-' : 'Rp '.number_format($amount, 0, ',', '.');
            ...
            <x-ledger-filter :action="route('trialBalance')" :offices="$offices" :selected-office="$selectedOffice"
                :start-date="$startDate" :end-date="$endDate" />

            @if ($rows->isEmpty())
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
                    <p class="text-yellow-800 font-semibold">Belum ada jurnal terposting sampai {{ $endDate->format('d M Y') }}</p>
                </div>
            @else
                @unless ($selectedOffice)
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
                        ...
                        <p class="text-sm text-yellow-600 mt-1">
                            Jurnal RAK di pusat dan cabang dibentuk berpasangan, sehingga jumlah seluruh akun RAK bernilai nol saat
                            semua kantor digabung. Akun "RAK Kantor Pusat" menampung buku semua cabang.
                        </p>
                    </div>
                @endunless
                            ...
                            <tbody class="text-gray-700 text-sm">
                                @foreach ($rows as $row)
                                    <tr class="hover:bg-gray-50 transition duration-150">
                                        ...
                                        <td class="p-4">
                                            <a href="{{ $ledgerLink($row['account']->id) }}" class="font-semibold hover:text-indigo-600"
                                                title="Lihat buku besar">{{ $row['account']->name }}</a>
                                            @if ($row['account']->isInterOffice())
                                                <span
