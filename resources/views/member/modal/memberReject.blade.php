...
<div id="rejectModal"
    ...
    <div class="bg-white rounded-2xl p-8 w-full max-w-lg shadow-2xl relative">
        ...
        <h2 class="text-2xl font-bold mb-6 text-gray-800 flex items-center gap-2">
            <i class="fas fa-user-times text-red-600"></i> Tolak Pendaftaran
        </h2>

        <form action="{{ route('rejectMember', $member['id']) }}" method="POST" class="space-y-5">
            @csrf
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-sm text-yellow-800">
                Pendaftaran <span class="font-bold">{{ $member['name'] }}</span> akan ditolak dan alasannya tercatat di
                riwayat keanggotaan.
            </div>
            ...
            <div>
                ...
                <textarea name="rejection_reason" rows="3" maxlength="200"
                    ...
                    required placeholder="e.g. Dokumen KTP tidak terbaca">{{ old('rejection_reason') }}</textarea>
            </div>

            <x-button type="submit" variant="primary" icon="save"
                class="w-full bg-slate-700 hover:bg-slate-800 justify-center">Tolak Pendaftaran</x-button>
        </form>
    </div>
</div>
